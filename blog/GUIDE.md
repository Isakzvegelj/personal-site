...
The blog is static. `blog/posts.js` drives the article list, while each new
post also has its own `blog/<slug>.html` page and sitemap entry. Older entries
without a `url` still open through `post.html?id=...`, but the publish tool is
recommended because it creates and stages all three pieces consistently.
...
copy-paste template. A manually added entry without a `url` remains supported
through the legacy query-page fallback; use the publisher for a static article
URL, full metadata, and automatic sitemap maintenance.
...
| `url`     | static page filename generated by the publisher; omit for legacy entries |
